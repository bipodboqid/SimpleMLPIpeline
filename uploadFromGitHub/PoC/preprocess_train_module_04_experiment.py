...
def preprocessing_fn(inputs):
    ...
    # image_tensor = tf.pad(image_tensor, [[10, 10], [0, 0], [0, 0], [0, 0]])
    # else:
        # pass
    ...
    # print(f'image_tensor\'s shape: {tf.shape(image_tensor)}')
    outputs[_FEATURE_KEY] = image_tensor
    # outputs[_LABEL_KEY] = inputs[_LABEL_KEY]
    label = tf.sparse.to_dense(inputs[_LABEL_KEY])
    # ここは[0]はいらない。SparseTensorのノリがわからないので理由は謎だが、とにかくいらない。
    # で、ラベルの形状は固定する必要がある。(batch_size, )の形状になるようreshapeする。
    # この辺も、正直完全に雰囲気でやっている。tfrecordがどのように格納してくるのか正直知らないのだ。
    label = tf.reshape(label, [-1])
    # print(f'label_tensor\'s shape: {tf.shape(label)}')
